...
public class MessagesStorage extends BaseController {
    ...
    public void openDatabase(int openTries) {
        ...
        AndroidUtilities.runOnUIThread(() -> {
                        showClearDatabaseAlert = false;            NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.onDatabaseOpened);
        });
    }
        ...
        storageQueue.postRunnable(() -> {
            ...
            try {
                ...
                while (filtersCursor.next()) {
                    ...
                    filter.pendingUnreadCount = filter.unreadCount = -1;                    filter.flags = filtersCursor.intValue(3);
        ...
        if (oldMessageId == newId && date != 0) {
            ...
            if (scheduled == 0) {
                try {
                    ...
                    state2.bindInteger(1, newId);                       state2.bindInteger(2, oldMessageId);
                    state2.bindLong(3, did);
                    state2.step();
                } catch (Exception e) {
                    try {
        ...
        try {
            ...
            if (quickReplies) {
                ...
                if (SharedConfig.keepDeletedMessages) {
                    if (dialogId != 0) {
                        for (int a = 0, N = messages.size(); a < N; a++) {
                            SharedConfig.markMessageDeleted(dialogId, messages.get(a));
                        }
                        dialogsIds.add(dialogId);
                    } else {
                        String ids = TextUtils.join(",", messages);
                        ...
                        while (cursor.next()) {
                            long did = cursor.longValue(0);
                            int mid = cursor.intValue(1);
                            SharedConfig.markMessageDeleted(did, mid);
                            if (!dialogsIds.contains(did)) {
                                dialogsIds.add(did);
                            }
                        }
                        cursor.dispose();
    ...
    public void loadMessageAttachPaths(ArrayList<MessageObject> messageObjects, Runnable done) {
        storageQueue.postRunnable(() -> {
            ...
            for (MessageObject messageObject : messageObjects) {
                if (messageObject.scheduled) continue;                 if (messageObject.isQuickReply()) continue;
                SQLiteCursor cursor = null;
                try {
